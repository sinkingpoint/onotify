...
export const getAllSilences = (store: DurableObjectStorage): Promise<Map<string, Silence>> => {
	return store.list({ prefix: `${SILENCE_KV_PREFIX}-` });
};

export const getAllAlerts = (store: DurableObjectStorage): Promise<Map<string, CachedAlert>> => {
	return store.list({ prefix: `${ALERT_KV_PREFIX}-` });
};

export const getAllAlertGroups = (store: DurableObjectStorage): Promise<Map<string, AlertGroup>> => {
