...
const firingAlert = (labels: Record<string, string>, annotations: Record<string, string>): Alert => {
	return {
...
test("silences don't match unanchored =~", async () => {
	...
	const storage = new SilenceDB(new MockSilenceStorage());
	await storage.addSilence(activeSilence('test=~"true"'));
	expect(storage.isSilenced(firingAlert({ test: "true-nottrue" }, {}))).toEqual(false);
});
...
test("silences doesn't update same", async () => {
	...
	expect(await storage.addSilence({ ...activeSilence('test=~"true"'), id })).toEqual([false, id]);
});
