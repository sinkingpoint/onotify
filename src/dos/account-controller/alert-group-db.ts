...
export class AlertGroupDB {
	...
	async getAlertGroups({ receiver, filter }: Partial<Pick<GetAlertGroupsOptions, "receiver" | "filter">>) {
	...
	async mergeAlertGroup(newGroup: AlertGroup) {
		const currentGroup = await this.getAlertGroup(newGroup.nodeID, newGroup.labelValues);
