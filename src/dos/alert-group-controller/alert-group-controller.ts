...
export class AlertGroupController extends DurableObject<Bindings> {
	...
	constructor(state: DurableObjectState, env: Bindings) {
		...
		state.blockConcurrencyWhile(async () => {
			...
			const [pending_alerts, active_alerts] = await getAlertFingerprints(this.ctx.storage);
			this.state_machine.initialise(pending_alerts, active_alerts);
		});
	}

	async initialize(account_id: string, route: FlatRouteConfig, group: AlertGroup) {
