...
const firingAlert = (labels: Record<string, string>): DehydratedAlert => {
	return {
...
const resolvedAlert = (labels: Record<string, string>): DehydratedAlert => {
	return {
		fingerprint: fingerprint(labels).toString(16),
...
test("state machine paginates", async () => {
	...
	expect(await state.flushPendingAlerts(1)).toEqual([{ ...alert, state: AlertState.Firing }]);
	expect(await state.flushPendingAlerts(1)).toEqual([{ ...alert2, state: AlertState.Firing }]);
});
