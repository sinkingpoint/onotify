...
const truncateAlerts = (
  alertFingerprints: CachedAlert[],
  n: number
): [CachedAlert[], number] => {
...
export default async (
  ...
  const status = alerts.some((a) => alertState(a) === AlertState.Firing)
    ? "firing"
    : "resolved";
  ...
  const payload: WebhookReceiverPayload = {
    ...
    alerts: newAlerts.map((a) => {
      const status =
        alertState(a) === AlertState.Firing ? "firing" : "resolved";
      return {
        ...a,
