...
export class GetRequiredFiles extends OpenAPIRoute {
  ...
  async handle(c: Context<{ Bindings: Bindings }>) {
    ...
    const rawRequiredFiles = await c.env.CONFIGS.get(
      requiredFilesKey(accountID)
    );
    const rawAlreadyUploadedFiles = await c.env.CONFIGS.get(
      uploadedFilesKey(accountID)
    );
    if (!rawRequiredFiles) {
    ...
    const alreadyUploadedFiles: Record<string, string> = JSON.parse(
      rawAlreadyUploadedFiles
    );

    requiredFiles.templates = requiredFiles.templates.filter(
      (t) => !alreadyUploadedFiles[t.path]
    );

    requiredFiles.secrets = requiredFiles.secrets.filter(
      (s) => !alreadyUploadedFiles[s]
    );
