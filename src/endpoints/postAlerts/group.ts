import { collapseRoutingTree, FlatRouteConfig, RouteConfig } from "../../types/alertmanager";
...
const groupAlert = (groups: Record<string, AlertGroup[]>, nodeID: string, node: FlatRouteConfig, alert: Alert) => {
	if (!node.receiver) {
	...
	const labels = node.group_by?.map((labelName) => alert.labels[labelName] ?? "") ?? [];

	const groupIdx = groups[nodeID].findIndex(
		(g) => g.labelValues.length === labels.length && g.labelValues.every((n, i) => labels[i] === n),
	);
...
const doesAlertMatchRoute = (a: Alert, r: Pick<RouteConfig, "match" | "match_re" | "matchers">) => {
