...
type PartitionedCookieConstraint = { partitioned: true; secure: true } | { partitioned?: boolean; secure?: boolean }; // reset to default
...
interface GetCookie {
	(r: HonoRequest, key: string): string | undefined;
	(r: HonoRequest): Cookie;
	(r: HonoRequest, key: string, prefixOptions?: CookiePrefixOptions): string | undefined;
}

export const getCookie: GetCookie = (r, key?, prefix?: CookiePrefixOptions) => {
	const cookie = r.raw.headers.get("Cookie");
...
export const parse = (cookie: string, name?: string): Cookie => {
	...
	for (let pairStr of pairs) {
		...
		if (validCookieValueRegEx.test(cookieValue)) {
			parsedCookie[cookieName] = cookieValue.indexOf("%") !== -1 ? tryDecode(cookieValue) : cookieValue;
			if (name) {
