...
test("Full Config", () => {
	const config = AlertmanagerConfigSpec.parse(readYamlFile("testdata/simple.yaml"));
	...
	expect(config.route.routes![0].group_by).toEqual(["alertname", "cluster", "service"]);
...
test("lots-of-required-files getRequiredFiles", () => {
	const config = AlertmanagerConfigSpec.parse(readYamlFile("testdata/lots-of-required-files.yaml"));
