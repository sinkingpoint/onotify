...
import { ArrowUpTrayIcon, CheckCircleIcon, QuestionMarkCircleIcon, XCircleIcon } from '@heroicons/react/16/solid';
import { AlertmanagerConfig, AlertmanagerConfigSpec, collapseRoutingTree, FlatRouteConfig } from '../../pkg/types/alertmanager';
...
const ConfigUpload = () => {
	...
	const [uploadingStatus, setUploadingStatus] = useState<UploadingStatus>(UploadingStatus.NotUploaded);
	...
	if(parseStatus?.config) {
		uploadButton = <button class="p-2 bg-green-600 rounded my-3" onClick={upload}>
			{uploadIcon} Let's go!
		</button>;
	}

	return <span class='flex flex-row grow'>
			<span class="flex flex-col grow">
				<span class="text-lg my-3">
					<h3>Paste your Alertmanager config, or {' '}
						<label for="config-file" class="btn">upload one</label>
					</h3>
				</span>
				<span class="flex flex-row grow">
					<span class="flex flex-row basis-2/3 grow">
						<input id='config-file' type='file' accept='.yml,.yaml' ref={uploadInputRef} onChange={openFile} style="display: none;"/>
						<textarea value={config} class="config-input grow mb-5 mr-5 p-3" ref={configTextBoxRef} onInput={handleConfigChange} />
					</span>
...
const ConfigStatus = ({parseError, config}: ConfigStatusProps) => {
	if(parseError) {
		return <><XCircleIcon class="size-6 inline" style={{color: "red"}} />Hrmm... That doesn't look like valid YAML...</>;
	} else if(config){
		...
		let templatesStrIcon = <CheckCircleIcon class="size-6 inline bg-green-600"/>;
		if(config.templates?.length) {
			templatesStrIcon = <QuestionMarkCircleIcon class="size-6 inline" style={{color: "yellow"}}/>;
		}

		return <>
			<ul>
				<li><CheckCircleIcon class="size-6 inline text-green-600"/> {numReceiversStr}</li>
				<li><CheckCircleIcon class="size-6 inline text-green-600"/> {numRoutesStr}</li>
				<li>{templatesStrIcon} {numTemplatesStr}</li>
				<li><CheckCircleIcon class="size-6 inline text-green-600"/> {numInhibitionsStr}</li>
				<li><CheckCircleIcon class="size-6 inline text-green-600"/> {numMuteTimesStr}</li>
			</ul>
		</>
	} else {
		return <></>;
