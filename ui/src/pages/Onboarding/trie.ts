...
const collapseTrie = (parent: trieNode) => {
	...
	if (parent.children.length > 1) {
		...
		const allUploaded = parent.children.every((c) => c.uploaded === UploadStatus.Uploaded);

		const someUploading = parent.children.some((c) => c.uploaded === UploadStatus.Uploading);
