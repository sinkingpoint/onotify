...
type StatusPaneProps = {
	fingerprint: string;
	alertPull: DataPull<GetAlertsResponse, unknown>;
};
...
export default ({ alertPull, fingerprint }: StatusPaneProps) => {
	...
	if (alertPull.state === "error") {
		return <InfoBox text="Failed to get alert" style="error" />;
	} else if (alertPull.state === "success" && alertPull.result.length === 0) {
		return <InfoBox text={`No such alert: ${fingerprint}`} style="error" />;
	} else {
		return (
			<>
